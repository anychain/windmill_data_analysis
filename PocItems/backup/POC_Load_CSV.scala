...
object POC_Load_CSV {
  def main(args: Array[String]) {
    if (args.length != 3) {
      System.err.prfloatln("Syntax: POC_Load_CSV <Spark Master URL> <HDFS URL> <Source location>")
      System.exit(1)
    }
  ...
  def load_csv(hdfs: String, dir: String, sc: SQLContext) = {
    ...
    val source = hiveContext.read.format("com.databricks.spark.csv").options(opts).load(hdfs 
      + dir)
    ...
    val df = sqlContext.createDataFrame(row_rdd, getSchema())

    df.na.drop().write.mode(SaveMode.Overwrite).save(hdfs + "/poc_merge_tmp")
    val tmp = hiveContext.read.load(hdfs + "/poc_merge_tmp/*")
    ...
    hiveContext.sql("insert overwrite table poc_merge_part "
      + "partition (C1) "
